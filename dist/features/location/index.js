...
exports.registerLocationCommands = void 0;
const axios_1 = __importDefault(require("axios"));
const path = __importStar(require("path"));
const fs = __importStar(require("fs-extra"));
// In-memory storage for user measurement states
const userMeasurementStates = new Map();
...
const markMessageAsProcessed = (userId, messageId) => {
...
const registerLocationCommands = (bot, deps) => {
    ...
    const initUserMeasurementState = (userId) => {
    ...
    const checkAndCleanupMeasurementState = (userId) => {
        const state = getUserMeasurementState(userId);
        if (!state)
            return false;
        const now = Date.now();
    ...
    const getRoute = async (startLat, startLon, endLat, endLon, transportMode = 'driving-car') => {
        try {
            if (!ORS_API_KEY) {
            ...
            const response = await axios_1.default.get('https://api.openrouteservice.org/v2/directions/' + transportMode, {
                params: {
                    api_key: ORS_API_KEY,
    ...
    const sendMeasurementResults = async (chatId, userId, firstPoint, secondPoint, distance, duration, transportMode) => {
        try {
            ...
            await bot.sendMessage(chatId, `📏 <b>Hasil Pengukuran</b> (Mode: ${transportText})\n\n` +
            ...
            if (transportMode !== 'car')
                alternateModes.push("/ukur_mobil");
            if (transportMode !== 'motorcycle')
                alternateModes.push("/ukur_motor");
            if (transportMode !== 'foot')
                alternateModes.push("/ukur");
            if (alternateModes.length > 0) {
                // Gunakan setTimeout dengan jeda singkat untuk menghindari race condition
                setTimeout(() => {
                    bot.sendMessage(chatId, `Anda dapat menggunakan ${alternateModes.join(' atau ')} dengan koordinat yang sama untuk melihat hasil lainnya. Data koordinat akan tersimpan selama 30 detik.`);
                }, 500);
            }
            ...
            bot.sendMessage(chatId, 'Terjadi kesalahan saat menampilkan hasil pengukuran. Silakan coba lagi.');
        }
    };
    // Helper function to check if last measurement is still valid
    const getLastMeasurement = (userId) => {
    ...
    const performStoredMeasurement = async (chatId, userId, firstPoint, secondPoint, transportMode) => {
        try {
            // Show that calculation is in progress
            const calculatingMsg = await bot.sendMessage(chatId, `Menghitung rute ${transportMode === 'car' ? 'mobil' :
                transportMode === 'motorcycle' ? 'sepeda motor' :
            ...
            const routeInfo = await getRoute(firstPoint.latitude, firstPoint.longitude, secondPoint.latitude, secondPoint.longitude, orsProfile);
            // Send measurement results
            await sendMeasurementResults(chatId, userId, firstPoint, secondPoint, routeInfo.distance, routeInfo.duration, transportMode);
        }
        catch (error) {
            console.error('Error performing stored measurement:', error);
            bot.sendMessage(chatId, 'Terjadi kesalahan saat menghitung ulang rute. Silakan coba kembali dengan /ukur.');
        }
    };
    ...
    bot.onText(/\/lokasi/, (msg) => {
        ...
        bot.sendMessage(msg.chat.id, 'Anda sekarang dalam mode Lokasi. Perintah yang tersedia:\n' +
    ...
    bot.onText(/\/alamat (.+)/, async (msg, match) => {
        ...
        if (currentMode !== 'location') {
            bot.sendMessage(msg.chat.id, 'Anda harus berada dalam mode Lokasi untuk menggunakan perintah ini. Ketik /lokasi untuk masuk ke mode Lokasi.');
            return;
        }
        ...
        try {
            ...
            if (response.data && response.data.length > 0) {
                ...
                bot.sendMessage(msg.chat.id, `📍 Koordinat untuk "${display_name}":\n` +
                    `Latitude: ${lat}\n` +
                    `Longitude: ${lon}\n\n` +
    ...
    bot.onText(/\/koordinat\s+(-?\d+\.?\d*)\s+(-?\d+\.?\d*)/, async (msg, match) => {
        ...
        try {
            ...
            if (response.data && response.data.display_name) {
                ...
                // Send location
                bot.sendLocation(msg.chat.id, parseFloat(lat), parseFloat(lon));
                bot.sendMessage(msg.chat.id, `📍 Alamat untuk koordinat (${lat}, ${lon}):\n` +
                    `${display_name}\n\n` +
                    `Untuk melihat di peta, ketik:\n` +
                    `/show_map ${lat},${lon}`);
            }
    ...
    bot.onText(/\/show_map (.+)/, async (msg, match) => {
        ...
        try {
            ...
            if (coordsMatch) {
                ...
                bot.sendMessage(msg.chat.id, `🗺️ Lihat di OpenStreetMap:\n${osmUrl}`);
            }
            else {
                ...
                if (response.data && response.data.length > 0) {
                    ...
                    // Also send a link to OpenStreetMap
                    const osmUrl = `https://www.openstreetmap.org/?mlat=${lat}&mlon=${lon}&zoom=15`;
                    bot.sendMessage(msg.chat.id, `🗺️ ${location.display_name}\n\nLihat di OpenStreetMap:\n${osmUrl}`);
                }
                else {
    ...
    bot.onText(/^\/ukur$/, (msg) => {
        ...
        const currentMode = getUserMode(userId);
        if (currentMode !== 'location') {
            bot.sendMessage(chatId, 'Anda harus berada dalam mode Lokasi untuk menggunakan perintah ini. Ketik /lokasi untuk masuk ke mode Lokasi.');
            return;
        }
        ...
        if (lastMeasurement) {
            ...
            performStoredMeasurement(chatId, userId, lastMeasurement.firstPoint, lastMeasurement.secondPoint, 'foot');
        }
        else {
    ...
    bot.onText(/^\/ukur_motor$/, (msg) => {
        ...
        const measurementState = getUserMeasurementState(userId);
        if (measurementState && measurementState.isActive) {
            bot.sendMessage(chatId, 'Anda sedang dalam proses pengukuran. Silakan selesaikan atau ketik /batal untuk membatalkan.');
            return;
        }
        // Try to get last measurement
        const lastMeasurement = getLastMeasurement(userId);
        if (lastMeasurement) {
            ...
            performStoredMeasurement(chatId, userId, lastMeasurement.firstPoint, lastMeasurement.secondPoint, 'motorcycle');
        }
        else {
            // No stored coordinates, start a new measurement
            bot.processUpdate({
    ...
    bot.onText(/^\/ukur_mobil$/, (msg) => {
        ...
        if (lastMeasurement) {
            ...
            performStoredMeasurement(chatId, userId, lastMeasurement.firstPoint, lastMeasurement.secondPoint, 'car');
        }
        else {
            // No stored coordinates, start a new measurement
            bot.processUpdate({
    ...
    const startNewMeasurement = (msg, defaultMode = 'foot', transportArg) => {
        const userId = msg.from?.id;
        if (!userId || !isUserRegistered(userId)) {
            bot.sendMessage(msg.chat.id, 'Maaf, Anda tidak terdaftar untuk menggunakan bot ini.');
            return;
        ...
        if (checkAndCleanupMeasurementState(userId)) {
            bot.sendMessage(msg.chat.id, 'Sesi pengukuran sebelumnya telah kedaluwarsa dan direset.');
        }
        ...
        bot.sendMessage(msg.chat.id, `📏 *Pengukuran Jarak dan Rute* (Mode: ${transportText})\n\n` +
            ...
            'Ketik /batal untuk membatalkan pengukuran.', { parse_mode: 'Markdown' });
    };
    ...
    bot.onText(/\/ukur\s+(-?\d+\.?\d*),\s*(-?\d+\.?\d*)\s+(-?\d+\.?\d*),\s*(-?\d+\.?\d*)/, async (msg, match) => {
        ...
        try {
            ...
            const [response1, response2] = await Promise.all([
                axios_1.default.get('https://nominatim.openstreetmap.org/reverse', {
            ...
            // Send results
            bot.sendMessage(msg.chat.id, `📏 *Hasil Pengukuran*\n\n` +
                `*Titik Awal:*\n${address1}\n(${lat1}, ${lon1})\n\n` +
                `*Titik Akhir:*\n${address2}\n(${lat2}, ${lon2})\n\n` +
                `*Jarak:* ${formattedDistance}\n\n` +
                `*Lihat Rute:*\n${osmRouteUrl}`, { parse_mode: 'Markdown' });
    ...
    bot.on('location', async (msg) => {
        ...
        const currentMode = getUserMode(userId);
        if (currentMode !== 'location') {
            bot.sendMessage(msg.chat.id, 'Anda harus berada dalam mode Lokasi untuk menggunakan fitur ini. Ketik /lokasi untuk masuk ke mode Lokasi.');
            return;
        }
        if (!msg.location) {
        ...
        if (measurementState && measurementState.isActive) {
            // Check for expired measurement state
            if (checkAndCleanupMeasurementState(userId)) {
                bot.sendMessage(msg.chat.id, 'Sesi pengukuran Anda telah kedaluwarsa. Silakan mulai pengukuran baru dengan perintah /ukur.');
                return;
            }
            ...
            try {
                ...
                if (!measurementState.firstPoint) {
                    ...
                    bot.sendMessage(msg.chat.id, `📍 Titik pertama diterima:\n${address}\n(${latitude}, ${longitude})\n\nSilakan kirim titik kedua.`);
                }
                // If first point is set but second is not, set second and calculate
                    ...
                    // Show that calculation is in progress
                    const calculatingMsg = await bot.sendMessage(msg.chat.id, `Menghitung rute ${transportMode === 'car' ? 'mobil' :
                        transportMode === 'motorcycle' ? 'sepeda motor' :
                    ...
                    // Get route information
                    const routeInfo = await getRoute(firstPoint.latitude, firstPoint.longitude, latitude, longitude, orsProfile);
                    // Reset measurement state
                    initUserMeasurementState(userId);
                    // Send measurement results using the new function
                    await sendMeasurementResults(msg.chat.id, userId, firstPoint, { latitude, longitude, address }, routeInfo.distance, routeInfo.duration, transportMode);
                }
            }
            catch (error) {
        ...
        try {
            ...
            if (response.data && response.data.display_name) {
                ...
                fs.writeJsonSync(path.join(cacheDir, filename), locationData, { spaces: 2 });
                bot.sendMessage(msg.chat.id, `📍 Informasi Lokasi:\n` +
                    `Latitude: ${latitude}\n` +
                    `Longitude: ${longitude}\n\n` +
                    `Alamat: ${display_name}`);
            }
            else {
                bot.sendMessage(msg.chat.id, `📍 Koordinat Lokasi:\n` +
                    `Latitude: ${latitude}\n` +
                    `Longitude: ${longitude}\n\n` +
        ...
        if (measurementState && measurementState.isActive) {
            ...
            try {
                ...
                // If first point is set but second is not, set second and calculate
                    ...
                    // Get route information
                    const routeInfo = await getRoute(firstPoint.latitude, firstPoint.longitude, lat, lon, orsProfile);
                    // Reset measurement state
                    initUserMeasurementState(userId);
                    // Send measurement results using the new function
                    await sendMeasurementResults(msg.chat.id, userId, firstPoint, { latitude: lat, longitude: lon, address }, routeInfo.distance, routeInfo.duration, transportMode);
                }
                return;
            }
            catch (error) {
...
exports.registerLocationCommands = registerLocationCommands;
