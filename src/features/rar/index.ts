...
// Interface for tracking user upload state
...
const userUploadStates = new Map<number, UserUploadState>();

// In-memory storage for usage statistics
const userUsageStats = new Map<number, UsageStats>();

export const registerRarCommands = (
  bot: TelegramBot,
  deps: RarCommandDependencies
): void => {
  ...
  const initUserUploadState = (userId: number, mode: 'zip' | 'extract' | 'search' | null = null): void => {
  ...
  const getUserUploadState = (userId: number): UserUploadState | undefined => {
    return userUploadStates.get(userId);
  };

  // Helper function to add file to user upload state
  const addFileToUploadState = (userId: number, filePath: string): void => {
  ...
  const getUserStats = (userId: number): UsageStats => {
  ...
  const updateUserStats = (userId: number, update: Partial<UsageStats>): void => {
    const stats = getUserStats(userId);
    Object.assign(stats, { ...update, lastUsed: Date.now() });
  };

  // Helper function to clean up user files
  const cleanupUserFiles = async (userId: number, filesToKeep: string[] = []): Promise<void> => {
    try {
  ...
  bot.onText(/\/rar/, (msg) => {
    ...
    bot.sendMessage(
      msg.chat.id,
      'Anda sekarang dalam mode Arsip. Perintah yang tersedia:\n' +
  ...
  bot.onText(/\/zip/, (msg) => {
    ...
    if (currentMode !== 'rar') {
      bot.sendMessage(
        msg.chat.id,
        'Anda harus berada dalam mode Arsip untuk menggunakan perintah ini. Ketik /rar untuk masuk ke mode Arsip.'
      );
      return;
    }
    ...
    bot.sendMessage(
      msg.chat.id,
      'Silakan kirim file-file yang ingin Anda arsipkan. Setelah selesai, ketik /kirim untuk membuat file ZIP.'
    );
  });
  ...
  bot.onText(/\/extract/, (msg) => {
    ...
    bot.sendMessage(
      msg.chat.id,
      'Silakan kirim file arsip (ZIP atau RAR) yang ingin Anda ekstrak. Setelah selesai, ketik /kirim untuk mengekstrak file.'
    );
  });
  ...
  bot.onText(/\/stats/, (msg) => {
    ...
    bot.sendMessage(
      msg.chat.id,
      '📊 *Statistik Penggunaan Fitur Arsip*\n\n' +
  ...
  bot.onText(/\/cari (.+)/, async (msg, match) => {
    ...
    if (!uploadState || uploadState.mode !== 'search' || uploadState.files.length === 0) {
      bot.sendMessage(
        msg.chat.id,
        'Anda harus mengirim file arsip terlebih dahulu dengan perintah /search.'
      );
      return;
    }
    ...
    try {
      ...
      if (fileExt !== '.zip' && fileExt !== '.rar') {
        bot.sendMessage(
          msg.chat.id,
          'Format file tidak didukung. Hanya file ZIP dan RAR yang dapat dicari.'
        );
        return;
      }
      ...
      if (!result) {
        bot.sendMessage(
          msg.chat.id,
          `❌ Tidak ditemukan file yang cocok dengan pola: ${searchPattern}`
        );
        return;
      }
      ...
      bot.sendMessage(
        msg.chat.id,
        'Terjadi kesalahan saat mencari dalam arsip. Pastikan utilitas yang diperlukan terinstal di server.'
      );
    }
  });
  ...
  bot.onText(/\/kirim/, async (msg) => {
    ...
    if (!uploadState || !uploadState.mode) {
      bot.sendMessage(
        msg.chat.id,
        'Silakan gunakan /zip atau /extract terlebih dahulu untuk memulai proses.'
      );
      return;
    }
    ...
    if (uploadState.files.length === 0) {
      bot.sendMessage(
        msg.chat.id,
        'Anda belum mengirimkan file apapun. Silakan kirim file terlebih dahulu.'
      );
      return;
    }
    ...
    try {
      ...
      if (uploadState.mode === 'zip') {
        ...
        if (fileExt === '.zip') {
          ...
          bot.sendMessage(
            msg.chat.id,
            'Format file tidak didukung. Hanya file ZIP dan RAR yang dapat diekstrak.'
          );
          return;
        }
        ...
        bot.sendMessage(
          msg.chat.id,
          `✅ ${extractedFiles.length} file berhasil diekstrak dari arsip.`
        );
      }
    } catch (error) {
